...
should_create_zero_sized_btree(_, {_, Btree}) ->
    ?_assertMatch(0, cbt_btree:size(Btree)).
...
should_remove_all_keys(KeyValues, Btree) ->
    Keys = keys(KeyValues),
    {ok, Btree1} = cbt_btree:add_remove(Btree, [], Keys),
    {
...
should_continuously_add_new_kv(KeyValues, {_, Btree}) ->
    {Btree1, _} = lists:foldl(
        fun(KV, {BtAcc, PrevSize}) ->
            ...
            ?assert(cbt_btree:size(BtAcc2) > PrevSize),
            {BtAcc2, cbt_btree:size(BtAcc2)}
        end, {Btree, cbt_btree:size(Btree)}, KeyValues),
    {
...
should_continuously_remove_keys(KeyValues, {_, Btree}) ->
    ...
    {Btree2, _} = lists:foldl(
        fun({K, _}, {BtAcc, PrevSize}) ->
            ...
            ?assert(cbt_btree:size(BtAcc2) < PrevSize),
            {BtAcc2, cbt_btree:size(BtAcc2)}
        end, {Btree1, cbt_btree:size(Btree1)}, KeyValues),
    {
...
should_insert_keys_in_reversed_order(KeyValues, {_, Btree}) ->
    ...
    {Btree1, _} = lists:foldl(
        fun(KV, {BtAcc, PrevSize}) ->
            ...
            ?assert(cbt_btree:size(BtAcc2) > PrevSize),
            {BtAcc2, cbt_btree:size(BtAcc2)}
        end, {Btree, cbt_btree:size(Btree)}, KeyValuesRev),
    should_produce_valid_btree(Btree1, KeyValues).
...
should_be_empty(Btree) ->
    ?_assertEqual(cbt_btree:size(Btree), 0).
...
fold_reduce(Btree, Opts) ->
    ...
    FoldFun = fun(GroupedKey, Unreduced, Acc) ->
        {ok, [{GroupedKey, cbt_btree:final_reduce(Btree, Unreduced)} | Acc]}
    end,
    cbt_btree:fold_reduce(Btree, FoldFun, [],
...
test_key_access(Btree, List) ->
    ...
    {ok, _, {[], Length}} = cbt_btree:fold(Btree, FoldFun,
                                             {Sorted, 0}, [{dir, rev}]),
    ok.
...
test_lookup_access(Btree, KeyValues) ->
    ...
    lists:foreach(
        fun({Key, Value}) ->
            ...
            {ok, _, true} = cbt_btree:fold(Btree, FoldFun,
...
test_final_reductions(Btree, KeyValues) ->
    ...
    {ok, _, FoldLRed} = cbt_btree:fold(Btree, FoldLFun, 0,
                                          [{start_key, LStartKey}]),
    {ok, _, FoldRRed} = cbt_btree:fold(Btree, FoldRFun, 0,
