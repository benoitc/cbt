...
should_return_enoent_if_missed() ->
    ?_assertEqual({error, enoent}, cbt_file:open("not a real file")).

should_ignore_invalid_flags_with_open() ->
    ?_assertMatch({ok, _},
                  cbt_file:open(?tempfile(), [create, invalid_option])).
...
should_create_empty_new_files(Fd) ->
    ?_assertMatch({ok, 0}, cbt_file:bytes(Fd)).
...
should_increase_file_size_on_write(Fd) ->
    {ok, 0, _} = cbt_file:append_term(Fd, foo),
...
should_return_current_file_size_on_write(Fd) ->
    {ok, 0, _} = cbt_file:append_term(Fd, foo),
    {ok, Size} = cbt_file:bytes(Fd),
    ?_assertMatch({ok, Size, _}, cbt_file:append_term(Fd, bar)).
...
should_write_and_read_large_binary(Fd) ->
    ...
    {ok, Pos, _} = cbt_file:append_binary(Fd, BigBin),
    ?_assertMatch({ok, BigBin}, cbt_file:pread_binary(Fd, Pos)).

should_read_iolist(Fd) ->
    %% append_binary == append_iolist?
    %% Possible bug in pread_iolist or iolist() -> append_binary
    {ok, Pos, _} = cbt_file:append_binary(Fd, ["foo", $m, <<"bam">>]),
    {ok, IoList} = cbt_file:pread_iolist(Fd, Pos),
    ?_assertMatch(<<"foombam">>, iolist_to_binary(IoList)).

should_fsync(Fd) ->
    {"How does on test fsync?", ?_assertMatch(ok, cbt_file:sync(Fd))}.
...
should_truncate(Fd) ->
    ...
    {ok, Size} = cbt_file:bytes(Fd),
    BigBin = list_to_binary(lists:duplicate(100000, 0)),
    {ok, _, _} = cbt_file:append_binary(Fd, BigBin),
    ok = cbt_file:truncate(Fd, Size),
    ?_assertMatch({ok, foo}, cbt_file:pread_term(Fd, 0)).
...
should_write_and_read_atom_header(Fd) ->
    ...
    ?_assertMatch({ok, hello, HeaderPos}, cbt_file:read_header(Fd)).

should_write_and_read_tuple_header(Fd) ->
    {ok, _} = cbt_file:write_header(Fd, {<<"some_data">>, 32}),
    ?_assertMatch({ok, {<<"some_data">>, 32}, _}, cbt_file:read_header(Fd)).

should_write_and_read_second_header(Fd) ->
    {ok, 0} = cbt_file:write_header(Fd, {<<"some_data">>, 32}),
    {ok, 4096} = cbt_file:write_header(Fd, [foo, <<"more">>]),
    ?_assertMatch({ok, [foo, <<"more">>], 4096}, cbt_file:read_header(Fd)).

should_truncate_second_header(Fd) ->
    {ok, _} = cbt_file:write_header(Fd, {<<"some_data">>, 32}),
    {ok, Size} = cbt_file:bytes(Fd),
    {ok, _} = cbt_file:write_header(Fd, [foo, <<"more">>]),
    ok = cbt_file:truncate(Fd, Size),
    ?_assertMatch({ok, {<<"some_data">>, 32}, 0}, cbt_file:read_header(Fd)).
...
should_produce_same_file_size_on_rewrite(Fd) ->
    ...
    {ok, _} = cbt_file:write_header(Fd, [foo, <<"more">>]),
    ?_assertMatch({ok, Size2}, cbt_file:bytes(Fd)).
    ...
    ?_assertMatch(
        ...
        check_header_recovery(
            fun(Fd, RawFd, Expect, HeaderPos) ->
                ?assertNotMatch(Expect, cbt_file:read_header(Fd)),
                file:pwrite(RawFd, HeaderPos, <<0>>),
                ?assertMatch(Expect, cbt_file:read_header(Fd))
            end)
    ).
